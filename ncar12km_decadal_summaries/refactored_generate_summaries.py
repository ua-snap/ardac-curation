...
def make_output_filename(climvar, model, scenario, start_year, end_year):
...
def create_decadal_averages(input_dir, output_dir):
    ...
    for subgroup in variable_di[var_group].keys():
        ...
        for decade_start in tqdm(range(1950, 2100, 10), desc=f"Processing Decades"):
            ...
            for climvar in tqdm(variable_di[var_group][subgroup].keys(),  desc=f"Processing variables for {decade_start}s"):
                ...
                decadal_mean_of_monthly_summary = compute_monthly_summaries(decade_slice, subgroup, climvar)
                ...
                for mo in months:
                    ...
                    output_filename = make_output_filename(climvar, model, scenario, decade_start, decade_start + 9)
