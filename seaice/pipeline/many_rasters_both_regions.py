"""The script creates a unified Chukchi + Beaufort dataset for cases
   where both regions have data from one or more rasters. The output is 
   a merged raster that uses the time series maximum of the data from 
   each region. This is considered Case D: many rasters, both regions."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Execute raster unification Case C."
    )
    parser.add_argument(
        "-dt",
        "--datetime_dict",
    ...
    if len(beauf_srcs) == 1:
        ...
        beauf_to_merge_src = max_dir.joinpath("beaufort_time_max_of_" + str(k).split(' ')[0] + ".tif")
    ...
    if len(chuk_srcs) == 1:
        ...
        stack = np.dstack(data_arrs)
        time_max = np.nanmax(stack, axis=2)
        chuk_to_merge_src = max_dir.joinpath("chukchi_time_max_of_" + str(k).split(' ')[0] + ".tif")
