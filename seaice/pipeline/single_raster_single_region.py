"""The script creates a unified Chukchi + Beaufort dataset for cases
   where one region has data from a single raster, but the other region
   does not have any data. The output is merged raster using the data and the 
   mask from the region without data.
   This is considered Case B: single raster, single region."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Execute raster unification Case B."
    )
    parser.add_argument(
        "-dt",
        "--datetime_dict",
