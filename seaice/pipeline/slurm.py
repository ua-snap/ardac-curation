"""Helper functions for writing slurm scripts for data processing.
   Function should get called from the pipeline processing notebook.
   Each slurm function calls out to a Python script."""


def write_sbatch(ncpus, slurm_email, slurm_dir, ac_conda_env, fp, profile, project_dir, output_dir):
...
def write_case_b(ncpus, slurm_email, slurm_dir, ac_conda_env, dt, ts, project_dir, mask_dir, src_dir, output_dir):
    ...
    slurm_fp = slurm_dir.joinpath(str(ts).split(" ")[0] + "_single_raster_single_region.slurm")
    with open(slurm_fp, "w") as f:
        f.write(head + args)
...
def write_case_c(ncpus, slurm_email, slurm_dir, ac_conda_env, dt, ts, project_dir, mask_dir, max_dir, src_dir, output_dir):
    ...
    slurm_fp = slurm_dir.joinpath(str(ts).split(" ")[0] + "_many_rasters_single_region.slurm")
    with open(slurm_fp, "w") as f:
        f.write(head + args)

    return slurm_fp

def write_case_d(ncpus, slurm_email, slurm_dir, ac_conda_env, dt, ts, project_dir, mask_dir, max_dir, src_dir, output_dir):
    ...
    slurm_fp = slurm_dir.joinpath(str(ts).split(" ")[0] + "_many_rasters_both_regions.slurm")
    with open(slurm_fp, "w") as f:
        f.write(head + args)
